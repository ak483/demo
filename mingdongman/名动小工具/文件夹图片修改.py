...
def rename(img_folder,num):
    for img_name in os.listdir(img_folder):  # os.listdir()： 列出路径下所有的文件
        ...
        reimg = img_name.lstrip('0123456789')
        dst = os.path.join(img_folder,reimg) #dst： 修改后的目录名      注意此处str(num)将num转化为字符串,继而拼接
        num= num+1
        os.rename(src, dst) #用dst替代src


def main():
    img_folder0 = 'G:/2345下载 - 副本' #图片的文件夹路径    直接放你的文件夹路径即可
    num=1
