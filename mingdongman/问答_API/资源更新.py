...
class SectionClassAPI(PublicClass):
    ...
    def Update_Resource(
            self, resourceId: int,  resourceTitle='', downloadUrl='', downloadPwd='',
            labelList='', keywordsList='', descriptionStr='',contentStr='',
    ):
        """
        1、更新资源
    ...
    def Update_Tutorial(self, tutorialId: int, tutorialTitle='', contentStr='', labelList='', keywordsList='', descriptionStr=''):
        """
        1、更新教程
...
if __name__ == '__main__':
    ...
    mainExcelDict = pd.read_excel(
        r'C:\Users\Adminitrator03\Desktop\视频教程（修改）.xlsx', sheet_name=[
            'Worksheet'
        ]
    )
    IdList = mainExcelDict['Worksheet']['教程ID'].to_list()
    TitleList = mainExcelDict['Worksheet']['教程名称'].to_list()
    ...
    # a = ArticleClassAPI()
    # for i in range(20):
    # # b = a.Query_Article(articleIdInt=26415, newsHuabshiBool=True, releaseTimeInt=1662430569, inquireIndex=1, arcrankInt=1)
    #     b = a.Update_Article(articleIdInt=30532+i, newsHuabshiBool=False, releaseTimeInt=1662430569)
    #     print(b)
    ...
    for i in range(len(IdList)):
        b = a.Update_Tutorial(tutorialId=IdList[i], tutorialTitle=TitleList[i])
        print(b)
    c.append(b)
    # 资源
    # a = SectionClassAPI()
    ...
    # c = []
    # for i in range(len(IdList)):
    #     b = a.Update_Resource(resourceId=IdList[i],resourceTitle=TitleList[i])
    #     print(b)
    #     c.append(b)
    ...
    d.to_excel('教程更新.xlsx',sheet_name='第一段')
