...
数据入库支持关系型数据库、
非关系型数据库和CSV文件存储，
关系型数据库由SQLAlchemy框架实现；
非关系型数据库目前仅支持MongoDB数据库。
pyReptile简化入库方式，
只需将爬取的数据以字典格式传入即可实现入库操作。
'''
...
class DataStorage(object):
    def __init__(self,CONNECTION,**kwargs):
        ...
        if self.databaseType =='SQL':
            ...
            self.DBSession = self.connenct(CONNECTION)
    ...
    def table(self,tablename):
        ...
        #将类属性进行判断，符合sqlalchemy的字段定义到数据映射类
...
def update(self,value,condition= {}):
    ...
    if self.databaseType == 'SQL':
        #更新条件只设置了单个条件
       if condition:
           c = self.table.__dict__[list(condition.keys())[0]].in_(list(condition.value()))
