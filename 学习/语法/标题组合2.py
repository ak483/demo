...
mainExcelDict = pd.read_excel(
    FILE_PATH_DICT['平台统计'], sheet_name=[
        '地域词优先级1','产品词优先级1','命题组成优先级2第一顺位','命题组成优先级2第二顺位'
    ]
)
region_ExcelData = mainExcelDict['地域词优先级1']
product_ExcelData = mainExcelDict['产品词优先级1']
...
for i in range(len(statisticsDate2)):
    ...
    for j in range(len(compose_product)):
        temp_statisticsDate2.append(re.sub('产品', regionExcelData[j], statisticsDate2[i]))  # 第一顺位替换产品词

# statisticsDate2 = temp_statisticsDate2
# #填充第二顺位标题地域词
# temp_statisticsDate2 = []
# for i in range(len(statisticsDate2)):
#     # product = random.choice(compose_product)
...
for i in range(len(statisticsDate1)):
    # product = random.choice(compose_product)
    for j in range(len(regionExcelData)):
        temp_statisticsDate1.append(re.sub('产品', regionExcelData[j],statisticsDate1[i]))#第一顺位替换地域词
