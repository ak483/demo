...
for i in range(len(statisticsDate2)):
    ...
    statisticsDate2[i] = re.sub('{地域}', '【地域】', statisticsDate2[i])

after2_queue = queue.Queue()  # 创建一个空队列
for title in statisticsDate2:
...
for i in range(len(statisticsDate1)):
    # product = random.choice(compose_product)
    statisticsDate1[i] = re.sub('{地域}', '【地域】', statisticsDate1[i])
...
for i in range(len(temp_statisticsDate1)):  #取出一个产品词
    ...
    for j in range(len(regionExcelData)):  #用这个产品词加上15个地域词生成生成15个第一顺位的标题
        ...
        region_after = re.sub('{(.*?)}',key, after)#完成第一顺位产品词的替换

        #如果每个都有地域词则一直遍历，没有地域词的则完成这一轮一二顺位并跳出循环，继续下一个产品词
        if '【地域】' in region_after:
            region_after = re.sub('【(.*?)】', regionExcelData[j], after) #取出地域词替换上
        else:
            # 随机从第二顺位取九个标题

            title2 = re.sub('{(.*?)}', key, title2)  # 随机出来的第二顺位替换产品词
            title2 = re.sub('【(.*?)】', regionExcelData[j], title2) #地域词
