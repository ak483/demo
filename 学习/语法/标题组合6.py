...
FILE_PATH_DICT = {
    ...
    '数据表': r'D:\untitled1\demo\学习\语法\优先级2第一二顺位第四批.xls'
...
All_content = []
for r in range(len(regionExcelData)):
    for k in range(len(productExcelData)):

        a = mainExcelDict1[(mainExcelDict1['地域词'] == regionExcelData[r]) & (mainExcelDict1['关键词'] == productExcelData[k])]
        b = a['标题'].to_list()
        c = a['分类'].to_list()
        c = c[0]
