import sys, os, time, pprint, pyperclip, threading, logging, cv2
import pandas as pd
...
class ZcoolClass:
    ...
    def Selenium_Login(self, userName: str, userPass: str) -> webdriver.Chrome:
        ...
        while True:
            ...
            # 检查有没有进入到作品中心
            try:
                WebDriverWait(browser, 10, 1).until(EC.presence_of_all_elements_located((By.XPATH, '//div[contains(@class,"cardBox")]')))
                browser.get('https://my.zcool.com.cn/works')
                break
            except Exception:
                browser.get('https://www.zcool.com.cn/')
    ...
    def Works(self, browser: webdriver.Chrome, worksDataDict: dict, getImgUrl=False) -> str:
        ...
        try:
            ...
            if worksClass_ == '插画':
                ...
                # 选择其他漫画
                browser.find_elemen(By.XPATH,'//section[contains(@class,"listSubBox")]/ul/li[6]').click()
        ...
        # 添加描述说明
        browser.find_element(By.XPATH,'//*[@placeholder="作品说明"]').send_keys(str(worksDataDict['explainStr']))
        time.sleep(1)
        ...
        # 发布
        try:
            browser.find_element(By.XPATH,'//button[contains(@class,"hztFkq")]').click()
        except Exception:
        ...
        time.sleep(3)
        try:
            WebDriverWait(browser, 15, 3).until(EC.presence_of_all_elements_located((By.XPATH, '//button[contains(@class,"dcoOXk")]')))
            # 返回作品列表
...
def Mouse_Move(requestsImgTh: threading.Thread):
    ...
    pyperclip.copy(FILE_PATH_DICT['无水印图片']+f'/{worksNameStr}')
    pag.hotkey('ctrl', 'v')
    time.sleep(0.5)
    pag.press('enter')
    time.sleep(1)
...
if __name__ == '__main__':
    ...
    # a = HuaBanClass()
    # startId = 6682
    # for userName, userPass in [['18027110277', 'Mingdong1314..']]:
    #     # 请求数据
    #     successData = IllustrationWorkClassAPI().Query(articleIdInt=startId + 1, inquireIndex=20, arcrankInt=0)
    #     if isinstance(successData, str):
    #         logging.info(f'请求失败：{successData}')
